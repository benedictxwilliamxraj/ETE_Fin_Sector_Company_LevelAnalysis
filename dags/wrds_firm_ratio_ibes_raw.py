...
def fetch_wrds_data(**context):
    ...
    cur.execute("SELECT COUNT(*) FROM wrdsapps_finratio_ibes.firm_ratio_ibes WHERE public_date >= '2020-01-01'")
    ...
    while total_rows < MAX_ROWS:
        query = f"""
            SELECT * FROM  wrdsapps_finratio_ibes.firm_ratio_ibes where public_date > '2020-01-01'
...
def create_table_query(**context):
    ...
    cursor.execute(""" SELECT COUNT(*)
            FROM ALL_TABLES
            WHERE TABLE_NAME = 'FIN_RATIOS_IBES_RAW'
    ...
    if table_exists:
        ...
        create_stmt = generate_create_table_sql(df, 'FIN_RATIOS_IBES_RAW')
...
def insert_rows_query(**context):
    ...
    table_name = 'FIN_RATIOS_IBES_RAW'
...
with DAG("wrds_firm_ratio_ibes", start_date=datetime(2025,4,28),schedule_interval="@monthly",catchup=False, tags=["INGESTION","WRDS"]) as dag:
