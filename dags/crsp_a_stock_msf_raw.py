...
def fetch_wrds_data(**context):
    ...
    cur.execute("""SELECT COUNT(*) FROM crsp_a_stock.msf WHERE "date" >= '2020-01-01'""")
    ...
    while total_rows < MAX_ROWS:
        query = f"""
            SELECT * FROM  crsp_a_stock.msf where "date" > '2020-01-01'
            ORDER BY "date"
...
def create_table_query(**context):
    ...
    cursor.execute(""" SELECT COUNT(*)
            FROM ALL_TABLES
            WHERE TABLE_NAME = 'CRSP_A_STOCK_MSF_RAW'
    ...
    if table_exists:
        ...
        create_stmt = generate_create_table_sql(df, 'CRSP_A_STOCK_MSF_RAW')
...
def insert_rows_query(**context):
    ...
    table_name = 'CRSP_A_STOCK_MSF_RAW'
...
with DAG("crsp_a_stock_msf", start_date=datetime(2025,4,28),schedule_interval="@monthly",catchup=False, tags=["INGESTION","WRDS"]) as dag:
