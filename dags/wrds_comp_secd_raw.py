...
def fetch_wrds_data(**context):
    ...
    cur.execute("""SELECT COUNT(*) FROM comp_na_daily_all.secd where datadate>'2020-01-01'""")
    ...
    while total_rows < MAX_ROWS:
        query = f"""
            SELECT * FROM  comp_na_daily_all.secd where datadate>'2020-01-01'
...
def create_table_query(**context):
    ...
    cursor.execute(""" SELECT COUNT(*)
            FROM ALL_TABLES
            WHERE TABLE_NAME = 'COMP_SECD_RAW'
    ...
    if table_exists:
        ...
        create_stmt = generate_create_table_sql(df, 'COMP_SECD_RAW')
...
with DAG("wrds_comp_secd", start_date=datetime(2025,4,28),schedule_interval="@monthly",catchup=False, tags=["INGESTION","WRDS"]) as dag:
