...
from utilities.sql_functions import generate_create_table_sql, insert_rows_one_by_one ,create_oracle_connection, insert_in_batches
...
def fetch_wrds_data(**context):
    ...
    sql = """SELECT * FROM comp_na_daily_all.funda where datadate >='2020-01-01' """
...
def create_table_query(**context):
    ...
    cursor.execute(""" SELECT COUNT(*)
            FROM ALL_TABLES
            WHERE TABLE_NAME = 'COMP_NA_FUND_A_RAW'
    ...
    if table_exists:
        ...
        create_stmt = generate_create_table_sql(df, 'COMP_NA_FUND_A_RAW')
...
def insert_rows_query(**context):
    ...
    # insert_rows_one_by_one(df, 'COMP_NA_FUND_A_RAW',conn)
    insert_in_batches(df,'COMP_NA_FUND_A_RAW',conn,40)





with DAG("wrds_comp_na_funda", start_date=datetime(2025,4,28),schedule_interval="@monthly",catchup=False, tags=["INGESTION","WRDS"]) as dag:
